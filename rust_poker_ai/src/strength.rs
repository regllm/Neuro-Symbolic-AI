...
fn simulate_river_games(
    ...
    let prob_unit = 1.0 / f32::from(river_simulation_count);
...
fn simulate_turn_ehs_distributions(
    ...
    lookup: &card::LookupTable,
    river_simulation_count: u16,
    turn_simulation_count: u16,
    river_cluster_count: u16,
) -> Vec<f32> {
    ...
    let prob_unit = 1.0 / f32::from(river_simulation_count);
    let sub_prob_unit = 1.0 / f32::from(turn_simulation_count);
    let mut ehs: Vec<f32> = vec![0.0, 0.0, 0.0];

    let mut base_combo: Vec<i32> = turn_combo.to_vec();
    base_combo.push(0);
    let mut another_combo: Vec<i32> = base_combo.to_vec();

    let mut result: Vec<f32> = Vec::with_capacity(river_cluster_count as usize);
...
pub fn simulate_turn_hand_strengths(
    ...
    let mut result: Vec<Vec<f32>> = Vec::with_capacity(turn_combos_size);
